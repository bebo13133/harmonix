...
const CustomHsHeader = ({ sections, currentSectionIndex, onSectionChange, formData, topInset = 0 }) => {
  ...
  const calculateCompletionPercentage = (section) => {
    ...
    section.questions.forEach((question) => {
      const questionId = question.id;
      if (formData?.formSections?.[section.title]?.selectedStatuses?.[questionId]) {
        completedQuestions++;
      }
    });
  ...
  const currentSection = Array.isArray(sections) && sections.length > currentSectionIndex ? sections[currentSectionIndex] : null;

  // const sectionTitle = currentSection && currentSection[1] ? currentSection[1].title : 'Select Section';
  const sectionTitle = currentSection ? currentSection.title : 'Select Section';
  // const questionsCount = currentSection && currentSection[1] && currentSection[1].questions ? currentSection[1].questions.length : 0;
  const questionsCount = currentSection && currentSection.questions ? currentSection.questions.length : 0;
  ...
  return (
    <View style={styles.header}>
      ...
      <View style={styles.titleContainer}>
        ...
        <Text style={[styles.subtitle, applyFontToStyle({}, 'regular', 18)]}>{`${questionsCount} questions | ${completionPercentage}% complete`}</Text>
      </View>
    </View>
  );
};
