...
import { View, Text, TouchableOpacity, Animated, ScrollView, Platform, TextInput, FlatList } from 'react-native';
    ...
    <StyledView className='mb-7'>
      ...
      {isExpanded && (
        <StyledView
          ...
          style={[
            getShadowStyle(5),
            { borderBottomLeftRadius: 10, borderBottomRightRadius: 10, overflow: 'hidden', maxHeight: 250 }
          ]}
        >
          <StyledTextInput
...
const InspectionModal = ({ isVisible, onClose, onStartInspection }) => {
  ...
  const renderDetailsItem = ({ item }) => {
    switch (item.type) {
      case 'project':
        return <CustomPicker label='Project Number' value={selectedProject} setValue={handleProjectSelect} items={projects} />;
      case 'inspector':
        return <CustomPicker label='Inspection Completed in the Presence of' value={selectedInspector} setValue={setSelectedInspector} items={inspectors} />;
      case 'personInControl':
        return <CustomPicker label='Person in Control of Site' value={selectedPersonInControl} setValue={setSelectedPersonInControl} items={personsInControl} />;
      case 'projectDirector':
        return <CustomPicker label='Project Director' value={selectedProjectDirector} setValue={setSelectedProjectDirector} items={projectDirectors} />;
      case 'divisionalDirector':
        return <CustomPicker label='Divisional Director' value={selectedDivisionalDirector} setValue={setSelectedDivisionalDirector} items={divisionalDirectors} />;
      default:
        return null;
    }
  };
  ...
  const detailsData = [
    ...
    { type: 'inspector', key: 'inspector' },
    { type: 'personInControl', key: 'personInControl' },
    { type: 'projectDirector', key: 'projectDirector' },
    { type: 'divisionalDirector', key: 'divisionalDirector' },
  ];
