...
// import { DcFormQuestions } from './DcFormQuestions';
...
const DcFormCreate = ({ route }) => {
  const navigation = useNavigation();
  const { saveFormData } = useUser();
  const initialData = route.params || {};
  const insets = useSafeAreaInsets();

  const { documentControlQuestions } = useDatabase();
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const data = await documentControlQuestions.load();
        setQuestions(data);
      } catch (err) {
        console.log(err);
      }
  ...
  const renderQuestions = () => {
    ...
    const questionEntries = sections
      .slice(startIndex, endIndex)
      .map((section) => (
        <FormSection
          key={section.title}
          section={section}
          updateFormSection={(sectionData) => updateFormSection(section.title, sectionData)}
          savedSectionData={formData.formSections[section.title] || {}}
        />
      ));
    ...
    if (currentSectionIndex === sections.length - 1) {
      questionEntries.push(
        <StyledView className='mb-4' key='general-comments'>
          <StyledText style={applyFontToStyle({}, 'semibold', 18)} className='mb-2 text-white'>
            General Comments
          </StyledText>
          <StyledTextInput
        ...
        <StyledView className='mb-4' key='signature'>
          ...
          <SignatureField ref={signatureRef} onSign={(signature) => handleInputChange('signature', signature)} />
        </StyledView>,
        <View style={styles.buttonContainer} key='buttons'>
          <TouchableOpacity style={[styles.button, { backgroundColor: Colors.GREEN }]} onPress={handleSave} disabled={isLoading}>
